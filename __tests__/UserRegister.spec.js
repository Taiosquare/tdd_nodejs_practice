...
describe('User Registration', () => {
    ...
    // To skip other tests and focus on one test put 'f' in front of 'it'
    ...
    it('returns 502 Bad Gateway when sending email fails', async () => {
        ...
        //mockSendAccountActivation.mockRestore(); // necessary, else test fails
    });
...
describe('Internationalization', () => {
    ...
    it.each`
        ...
        ${'password'}  | ${'lower4nd5667'} | ${password_pattern}
        ${'password'}  | ${'UPPER44444'}   | ${password_pattern}
    `('returns $expectedMessage when $field is $value when language is set as turkish', async ({ field, value, expectedMessage }) => {
        const user = {
            username: 'user1',
            email: 'user1@mail.com',
            password: 'P4ssword'
...
describe('Account Activation', () => {
    ...
    it('returns bad request when token is wrong', async () => {
        ...
        const response = await request(app)
            .post('/api/1.0/users/token/' + token).send();
        expect(response.status).toBe(400);
    });
